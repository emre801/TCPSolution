...
When creating this server class, it needed to do two basic functions. First it would need to accept connections from Clients so that messages can be recieved. Second it would need a way to decode those messages in order to print the necessary information.  

When doing this, it’s normally pretty easier to serve connectivity, but when I thought about it further I needed a way to set up a safer connection. For example look at the following code

ServerSocket serverSocket= new ServerSocket(1234)
Socket connectionSocket = serverSocket.accept();

This does the basics, it sets up a connection between a server and client, then you can communication can take place. The downside to this is that this code could be subject to a middle man attack. Meaning that someone could read all incoming messages from the client and forward it to the server. All private information such as passwords or credit card numbers(payload) would be subject to an attack.

In order to accomplish security, I setup a SSL socket connection. We need atleast a basic level of encryption so that even if there was a middle man attack, the data he would be getting would be useless.
